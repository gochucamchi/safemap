# -*- coding: utf-8 -*-
"""
Kakao Local API를 사용한 주소 → 좌표 변환 서비스
- 한국 주소에 최적화
- Kakao Map과 동일한 API 사용으로 일관성 보장
"""
...
class KakaoGeocodingService:
    """Kakao Local API Geocoding 서비스"""

    def __init__(self, rest_api_key: str):
        """
        Args:
            rest_api_key: Kakao REST API 키
        """
    ...
    async def geocode_address(self, address: str) -> Optional[Tuple[float, float]]:
        ...
        try:
            ...
            print(f"⚠️  HTTP 오류: {address[:30]}, {str(e)}")
            return None
...
def get_geocoding_service(rest_api_key: str) -> KakaoGeocodingService:
    """지오코딩 서비스 인스턴스 반환"""
    global _geocoding_service
    if _geocoding_service is None:
        _geocoding_service = KakaoGeocodingService(rest_api_key)
